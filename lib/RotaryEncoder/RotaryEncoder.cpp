...
char RotaryEncoder::ENCPIN1=4;
bool RotaryEncoder::dstep=false;

RotaryEncoder::RotaryEncoder(bool rev, bool dblstep)
{
    enc=this;
    dstep=dblstep;
...
void RotaryEncoder::changeInterrupt()
{
    ...
    if(newstate != enc->laststate){
        ...
        if(p0 == p1){
            ...
            // previous one
            if(newstate != enc->lastdetent && (!dstep || newstate == 0))
                enc->delta+=enc->trans;
