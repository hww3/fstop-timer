...
Keypad::Keypad(char c0, char c1, char c2, char c3, char r0, char r1, char r2, char r3, bool rev)
{
    reverse=rev;
    if(rev){
        pins[0]=c3;
    ...
    for(char row=0;row<4;++row){
        ...
        for(char col=0;col<4;++col){
            if(!digitalRead(pins[4+col])){
                newsee=reverse ? ((col<<2)|row) : ((row<<2)|col);
            }
        }
...
SMSKeypad::SMSKeypad(char c0, char c1, char c2, char c3, char r0, char r1, char r2, char r3, bool rev)
    : Keypad(c0, c1, c2, c3, r0, r1, r2, r3, rev)
{
}
...
SMSKeypad::Context::Context(char *b, unsigned ml, LiquidCrystalFast *l,
